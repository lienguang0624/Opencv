...
def getFilelist(path):
    ...
    for file in files:
        filelist.append(os.path.join(path,file))#把目录和文件名合成一个路径
    return filelist
...
def findPoints(filelist,boardSize,board_distance):
    # 首先计算所有图像的角点位置
    imagePoints=[]
    imageSize=(0,0)
    for file in filelist:
        ...
        found , imageCorners=cv.findChessboardCorners(image,boardSize)#提取棋盘格中的角点
        imageCorners=cv.cornerSubPix(image, imageCorners, (11, 11), (-1, -1), (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 30, 0.001))#使用亚像素提高精度
        DEBUG=0
        if DEBUG==1:
    ...
    # 初始化世界角点坐标,棋盘格大小不同在这里进行更改
    objectCorner =[]
    for j in range(boardSize[1]):
        for i in range(boardSize[0]):
            objectCorner.append([i+board_distance,j+board_distance,0])
...
def main(boardSize,filelist,board_distance):
    # 输入：
    # filelist：图片文件路径名
    # boardSize：一个tuple(棋盘格行数，棋盘格列数)
    # 输出：
    # imagePoints：所有图像的角点位置list[array(boardSize)]
    # objectCorners：世界角点坐标array(boardSize)
    imagePoints, objectCorners, imageSize = findPoints(filelist, boardSize,board_distance)
    cameraMatirx, disCoeffs, rvecs, tvecs = cv_calibrate(imagePoints, objectCorners, imageSize)
    save('./result/singleCal', cameraMatirx, disCoeffs, rvecs, tvecs)
...
if __name__=='__main__':
    ...
    filelist：图片文件路径名
    '''
    filelist = getFilelist('C:\\Users\\lieng\\OneDrive\\Documents\\GitHub\\Opencv\\left')
    main((6,9),filelist,15)